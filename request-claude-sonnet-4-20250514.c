...
int do_restart(message *m_ptr)
{
  ...
  if (strlen(rp->r_script) >= sizeof(script)) {
    return EINVAL;
  }
...
int do_refresh(message *m_ptr)
{
    ...
    result = check_call_permission(m_ptr->m_source, RS_REFRESH, rp);
    if (result != OK) {
        return result;
    }
...
int do_init_ready(message *m_ptr)
{
  ...
  if (who_p < 0 || who_p >= NR_PROCS || !rproc_ptr[who_p]) {
      return EINVAL;
  }
...
int do_update(message *m_ptr)
{
  ...
  lu_flags = extract_lu_flags(&rs_start);
  init_flags = extract_init_flags(&rs_start) | lu_flags;

  if(should_set_default_prealloc(&rs_start, rpub, lu_flags)) {
  ...
  prepare_maxtime = (m_ptr->m_rs_update.prepare_maxtime == 0) ? 
      RS_DEFAULT_PREPARE_MAXTIME : m_ptr->m_rs_update.prepare_maxtime;

  s = validate_update_constraints(rp, &rs_start);
  if(s != OK) {
      return s;
  }
  ...
  if(!(rs_start.rss_flags & RSS_PREPARE_ONLY_LU)) {
      s = create_new_service_version(rp, &rs_start, m_ptr->m_source, &new_rp);
      if(s != OK) {
          return s;
      }
      ...
      s = handle_memory_preallocation(new_rp, &rs_start);
      if(s != OK) {
          cleanup_service(new_rp);
          return s;
      }
  ...
  s = create_update_grants(rpupd, rpub);
  if(s != OK) {
      rupdate_upd_clear(rpupd);
      return s;
  }
  ...
  if(rs_start.rss_flags & RSS_BATCH) {
      return OK;
  }
  ...
  if(rs_start.rss_flags & RSS_NOBLOCK) {
      return OK;
  }
...
static int extract_lu_flags(const struct rs_start *rs_start)
{
    int lu_flags = 0;
...
static int extract_init_flags(const struct rs_start *rs_start)
{
    int init_flags = 0;
...
static int should_set_default_prealloc(const struct rs_start *rs_start, 
                                       const struct rprocpub *rpub, 
                                       int lu_flags)
{
    return (rs_start->rss_map_prealloc_bytes <= 0 &&
...
static int validate_update_constraints(const struct rproc *rp, 
                                      const struct rs_start *rs_start)
{
    ...
    if(prepare_only && 
       (rp->r_pub->endpoint == VM_PROC_NR || 
        ...
        if(m_ptr->m_rs_update.state != SEF_LU_STATE_UNREACHABLE) {
...
static int create_new_service_version(struct rproc *rp, 
                                     const struct rs_start *rs_start,
                                     endpoint_t source,
                                     struct rproc **new_rp)
{
    ...
    if(do_self_update) {
        ...
        s = clone_service(rp, LU_SYS_PROC, rp->r_upd.init_flags);
        if(s != OK) {
        ...
        *new_rp = rp->r_new_rp;
        ...
        s = init_slot(*new_rp, rs_start, source);
        if(s != OK) {
        ...
        (*new_rp)->r_priv.s_init_flags |= rp->r_upd.init_flags;
...
static int handle_memory_preallocation(struct rproc *new_rp, 
                                      struct rs_start *rs_start)
{
    int s;
    ...
    if(rs_start->rss_heap_prealloc_bytes) {
        ...
        if(new_rp->r_priv.s_flags & ROOT_SYS_PROC) {
...
static int create_update_grants(struct rprocupd *rpupd, 
                               const struct rprocpub *rpub)
{
...
void do_sigchld()
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        found = 0;
        if(get_service_instances(rp, &rps, &nr_rps) == 0) {
            for(i = 0; i < nr_rps; i++) {
                if(rps[i] != NULL) {
 ...
 *				   check_request			     *
 *===========================================================================*/
static int check_scheduler(struct rs_start *rs_start)
{
...
static int check_priority(struct rs_start *rs_start)
{
...
static int check_quantum(struct rs_start *rs_start)
{
...
static int check_cpu(struct rs_start *rs_start)
{
...
static int check_sigmgr(struct rs_start *rs_start)
{
...
static int check_request(struct rs_start *rs_start)
{
    ...
    result = check_scheduler(rs_start);
    if (result != OK) {
        return result;
    }
    
    result = check_priority(rs_start);
    if (result != OK) {
        return result;
    }
    
    result = check_quantum(rs_start);
    if (result != OK) {
        return result;
    }
    
    result = check_cpu(rs_start);
    if (result != OK) {
        return result;
    }
    
    result = check_sigmgr(rs_start);
    if (result != OK) {
        return result;
    }
